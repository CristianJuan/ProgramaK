...
public abstract class Unit : MonoBehaviour
{
    ...
    public FieldOfView fieldOfView;// To come from the inspector

    [Header("Weapon Systems")]
    public int numberOfWeapons = 1;
    public List<IWeaponSystem> weaponSystems;
    ...
    public virtual void DealDamage(Unit other)
    {
        ...
        Debug.Log(this.name + " Called its DealDamage function. this will call if conditions met the Defend method of the unit: " + other.name);
        if (isMoving)
            return;
        if (ActionPoints == 0)
            return;
        ...
        if(UseFieldOfView)
        {
            List<Unit> targets = fieldOfView.visibleTargetsInUnits;
            Debug.Log("In deal damage the other parameter transform name is: " + other.transform.name);
            Unit otherUnitInFOVList = targets.Find(u => u == other);
            if(otherUnitInFOVList != null)
            {
                Debug.Log("otherUnitInFOVList is not null, can deal damage is true, its name is: " + otherUnitInFOVList.name);
                canDealDamage = true;
            }
            else
            {
                Debug.Log("otherUnitInFOVList is null, can deal damage is false");
                canDealDamage = false;
            }
        }
        if(canDealDamage)
        ...
        {
            Debug.Log("can deal damage is false");
        }
    ...
    {
        Debug.Log("Turret:" + this.name+ " called base unit class Defend method by, the other parameter name is: " + other.name);
    ...
    protected virtual IEnumerator MovementAnimation(List<Cell> path)
    {
        ...
        this.transform.DOScale(new Vector3(0, 0, 0), 0.3f);
        path.Reverse();
        foreach (var cell in path)
        {
            ...
            while (transform.localPosition != destination_pos)
            {
                ...
                //this.transform.DOMove(destination_pos, Time.deltaTime * MovementSpeed * 0.1f).SetEase(Ease.InOutFlash); //NOT BAD FOR PROTOTYPE THE DO TWEEN DONT WORK WELL HERE.
                transform.localPosition = Vector3.MoveTowards(transform.localPosition, destination_pos, Time.deltaTime * MovementSpeed);
                yield return 0;
                //this.transform.DOMove(destination_pos, Time.deltaTime * MovementSpeed * 0.2f).SetEase(Ease.InOutFlash);//NOT BAD FOR PROTOTYPE THE DO TWEEN DONT WORK WELL HERE.
            }
        }
        isMoving = false;
